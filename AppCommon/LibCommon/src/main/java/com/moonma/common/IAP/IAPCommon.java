    ...
    public static void RestoreBuy(String product)
{
    ...
    public void onBuyDidFinish() {
        sActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                LibCommon.Main().UnitySendMessage(unityObjName,unityObjMethod,CALLBACK_BUY_DID_FINISH);
            }
        });
    }
    ...
    public void onBuyDidFail() {
        sActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                LibCommon.Main().UnitySendMessage(unityObjName,unityObjMethod,CALLBACK_BUY_DID_FAIL);
            }
        });
    }
    @Override
    public void onBuyDidBuy() {
        sActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                LibCommon.Main().UnitySendMessage(unityObjName, unityObjMethod, CALLBACK_DID_BUY);
            }
        });
    }
    ...
    public void onBuyDidRestore() {
        sActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                LibCommon.Main().UnitySendMessage(unityObjName,unityObjMethod,CALLBACK_BUY_DID_RESTORE);
            }
        });
